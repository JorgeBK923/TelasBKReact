...
export function AgentCard({ name, description, version, tags, icon, image, status, lastRun, isNew, isHighPriority }: AgentCardProps) {
    return (
        <div className="group relative flex flex-col bg-white dark:bg-[#1c2027] rounded-2xl border border-[#e2e8f0] dark:border-[#3b4554] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            ...
            <div className={`px-6 py-4 border-t flex items-center justify-between gap-4 ${status === 'offline' ? 'bg-red-50/50 dark:bg-red-900/10 border-red-100 dark:border-red-900/20' : 'bg-slate-50 dark:bg-[#15191f] border-[#e2e8f0] dark:border-[#3b4554]'}`}>
                <div className="flex items-center gap-2">
                    <div className={`h-2 w-2 rounded-full ${status === 'available' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500'}`}></div>
                    <span className={`text-xs font-bold ${status === 'offline' ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>
                        {status === 'available' ? 'Disponível' : 'Offline'}
                    </span>
                </div>
                {status === 'offline' ? (
                    <button className="flex-1 py-2.5 px-4 text-sm font-bold rounded-lg transition-all bg-transparent border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">
                        Reiniciar Agente
                    </button>
                ) : (
